...
export async function getAsteroid(id: string): Promise<Asteroid | null> {
    ...
    try {
        let res = await fetch(
            `${process.env.ASTEROID_URL}/${id}?api_key=${process.env.API_KEY}`
        );
...
export async function getOrderAsteroids(
    order: string[]
): Promise<Asteroid[] | null> {
    let asteroids: Asteroid[] = [];

    let res = await fetch("/api/asteroids/order", {
        method: "POST",
...
export function handleWeek(week: AsteroidWeek): Asteroid[] | null {
    ...
    try {
        let sortedDates = Object.keys(week.near_earth_objects).sort(
            (asDates1, asDates2) =>
                new Date(asDates2).valueOf() - new Date(asDates1).valueOf()
        );
...
export async function getAsteroids(
    d: Date
): Promise<{ asteroids: Asteroid[]; date: Date } | null> {
    let res = await fetch(`/api/asteroids/${formateDate(d)}`);
    let asteroidWeek: { asteroids: AsteroidWeek; date: Date } =
        await res.json();
