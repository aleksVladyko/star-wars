...
const AsteroidGrid: React.FC<AsteroidGridProps> = memo(
    ({ asteroids, initialDate, errorText, infiniteLoad }) => {
        ...
        const handleAsteroids = useCallback(async () => {
            ...
            window.scrollBy(0, -1);
        }, [currentDate, infiniteLoad, loadingMore, setAsteroids]);
        ...
        useEffect(() => {
            if (
                ...
                asteroids &&
                asteroids.filter(
                    (as) => as.is_potentially_hazardous_asteroid === true
                ).length < 16
            ) {
                handleAsteroids();
            }
        }, [asteroids, handleAsteroids, showHazardous]);
        ...
        return !asteroids || asteroids.length === 0 ? (
            ...
            <InfiniteScroll
                ...
                endMessage={
                    <h3 className={classes.noMoreAsteroids}>
                        Больше астероидов нет :с
                    </h3>
                }
            >
            ...
            <ul className={classes.asteroidsGrid}>
                ...
                {infiniteLoad ? (
                    <li>
                        <Button onClick={() => handleAsteroids()}>
                            Get more
                        </Button>
                    </li>
                ) : (
