...
const Asteroid: React.FC<AsteroidPageProps> = ({ apod, asteroid }) => {
    const { order, removeAsteroid, addAsteroid, selectedMetric } =
        useAsteroidContext();
    ...
    const isInOrder = useCallback(() => {
        return order.find((as: { id: string; }) => as.id === asteroid.id) !== undefined;
    }, [order]);
    ...
    const memoizedClosestDate = useCallback(
        () => closestDate(asteroid),
        [asteroid]
    );
    ...
    return (
        <Layout apod={apod}>
            <Head>
                <title>{asteroid.name} - StarWars</title>
            </Head>
            <HeaderSecondary
                                        ...
                                        Number(
                                            asteroid.close_approach_data.find(
                                                (as) =>
                                                    ...
                                                    memoizedClosestDate().valueOf()
                                            )!.miss_distance[
                                                selectedMetric === "kiloMeters"
                                                    ? "kilometers"
                                                    : "lunar"
                                            ]
                                        )
                                    ...
                                    <tbody>
                                        {asteroid.close_approach_data
                                            .sort(
                                                (app1, app2) =>
                                                    new Date(
                                                        app2.close_approach_date
                                                    ).valueOf() -
                                                    new Date(
                                                        app1.close_approach_date
                                                    ).valueOf()
                                            )
                                            .map((approach, i) => (
                                                <tr
                                                ...
                                                >
                                                    ...
                                                    <td>
                                                        {
                                                            Planets[
                                                                approach.orbiting_body as keyof typeof Planets
                                                            ]
                                                        }
                                                    </td>
                                                </tr>
